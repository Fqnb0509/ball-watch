...
export default function FieldWatchApp() {
  const { data: matches, loading: matchesLoading, error: matchesError, lastUpdated, provider: matchesProvider, fallback: matchesFallback, refresh: refreshMatches } = useMatches()
  ...
  const { data: streams, loading: streamsLoading, error: streamsError, status: streamsStatus, stale: streamsStale, fallbackActive: streamsFallbackActive, refresh: refreshStreams } = useStreams(selected)
...
function Home(props: HomeProps) {
  ...
  const source = props.fallback ? '演示数据 / fallback' : props.provider?.includes('api-football') ? 'API-Football' : '演示数据'
  const dataStatus = props.loading ? '正在加载赛事数据…' : props.error ? `数据刷新失败：${props.error} · 数据来源：${source}` : props.lastUpdated ? `最后更新 ${new Intl.DateTimeFormat('zh-CN', { timeZone: BEIJING_TIME_ZONE, hour: '2-digit', minute: '2-digit' }).format(props.lastUpdated)} · 数据来源：${source}` : 'Demo 赛事数据'
  return <><section className="fw-hero"><div><p className="fw-eyebrow"><i />LIVE SPORTS HUB</p><h1>你关注的比赛，<em>都在这里。</em></h1><p className="fw-hero-copy">清爽、专注、无干扰。赛事数据与直播源分层管理。</p></div><div className="fw-date"><span>北京时间</span><b>{formatDate(new Date().toISOString())}</b><small>实时赛程视图</small></div></section><section className="fw-sports" aria-label="体育项目筛选">{sportOrder.map((item) => <button key={item} className={props.sport === item ? 'fw-sport active' : 'fw-sport'} onClick={() => props.onSport(item)}><span>{sportIcons[item]}</span>{sportLabels[item]}</button>)}</section><section className="fw-schedule"><div className="fw-heading"><div><p className="fw-eyebrow muted">SCHEDULE</p><h2>{heading}</h2><p className="fw-data-status">{dataStatus}</p></div><div className="fw-heading-actions"><span>{props.list.length} 场比赛</span><select value={props.league} onChange={(event) => props.onLeague(event.target.value)} aria-label="赛事筛选"><option value="all">全部赛事</option>{props.leagues.map((item) => <option key={item} value={item}>{item}</option>)}</select><button className="fw-refresh" onClick={props.onRefresh} disabled={props.loading} aria-label="刷新赛事数据">{props.loading ? '…' : '刷新'}</button></div></div><div className="fw-days" aria-label="日期筛选">{dayItems.map(([mode, label, offset]) => <button key={mode} className={props.dayMode === mode ? 'selected' : ''} onClick={() => props.onDay(mode)}><small>{label}</small><b>{beijingDateParts(addDays(offset)).day}</b></button>)}<label className={props.dayMode === 'custom' ? 'fw-custom-date selected' : 'fw-custom-date'}><small>自定义</small><input type="date" value={props.customDate} onChange={(event) => props.onCustomDate(event.target.value)} aria-label="自定义日期" /></label></div>{props.query && <p className="fw-filter-summary">“{props.query}” 的搜索结果</p>}{props.view === 'home' && props.recentMatches.length > 0 && <section className="fw-recent"><div className="fw-group-title"><h3>最近观看</h3><span>自动保留最近 10 场</span></div><div className="fw-recent-list">{props.recentMatches.slice(0, 4).map((match) => <button key={match.id} onClick={() => props.onOpen(match)}>{match.homeTeam.name}<i>vs</i>{match.awayTeam.name}</button>)}</div></section>}{props.loading && !props.list.length ? <div className="fw-empty"><strong>正在加载赛事数据</strong><span>请稍候，正在读取缓存或 Demo 数据。</span></div> : props.list.length ? <div className="fw-groups"><MatchGroup title="正在进行" status="live" list={props.list} favorite={props.isFavorite} onToggleFavorite={props.onToggleFavorite} onOpen={props.onOpen} /><MatchGroup title="即将开始" status="upcoming" list={props.list} favorite={props.isFavorite} onToggleFavorite={props.onToggleFavorite} onOpen={props.onOpen} /><MatchGroup title="已结束" status="finished" list={props.list} favorite={props.isFavorite} onToggleFavorite={props.onToggleFavorite} onOpen={props.onOpen} /><MatchGroup title="已取消" status="cancelled" list={props.list} favorite={props.isFavorite} onToggleFavorite={props.onToggleFavorite} onOpen={props.onOpen} /><MatchGroup title="已延期" status="postponed" list={props.list} favorite={props.isFavorite} onToggleFavorite={props.onToggleFavorite} onOpen={props.onOpen} /><MatchGroup title="已暂停" status="suspended" list={props.list} favorite={props.isFavorite} onToggleFavorite={props.onToggleFavorite} onOpen={props.onOpen} /></div> : <EmptyState view={props.view} />}</section></>
}
...
function Watch(props: WatchProps) {
  ...
  const streamStatusMessage = props.streamsLoading
    ? '正在查询直播源'
    : props.streamsStatus === 'failure'
      ? '直播查询全部失败'
      : props.streamsFallbackActive
        ? '当前使用 fallback'
        : props.streamsStale || props.streamsStatus === 'stale'
          ? '正在使用缓存中的直播源'
          : props.streamsStatus === 'partial-failure'
